...
class RecipeGETSerializer(serializers.ModelSerializer):
    ...
    ingredient_amounts = IngredientFullSerializer(many=True, read_only=True)
    ...
    def get_ingredients(obj):
        ...
        ingredients = obj.ingredient_amounts.all()  # Используем related_name
    ...
    def get_is_favorited(self, obj):
        ...
        if request is None or request.user.is_anonymous:
            return False
        return request.user.favoriting.filter(recipe=obj).exists()
...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    def validate_ingredients(self, ingredients):
        ...
        for ingredient in ingredients:
            ...
            if int(ingredient.get('amount')) > Recipe.MAX_INGREDIENT_AMOUNT:
                raise serializers.ValidationError(
                    'Количество ингредиента не может быть больше '
                    + f'{Recipe.MAX_INGREDIENT_AMOUNT}'
                )
        return ingredients
    ...
    def create(self, validated_data):
        ...
        if not ingredients_data:
            raise serializers.ValidationError("Добавь ингредиенты")
